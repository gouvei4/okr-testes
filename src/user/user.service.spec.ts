...
describe('UserService', () => {
  ...
  describe('createUser', () => {
    it('should create a user eith valid data', () => {
    ...
    it('should throw if name is empty', () => {
      expect(() => service.createUser({ ...mockUser, name: ''}))
        .toThrow(BadRequestException);
    });

    it('should throw if CPF is invalid', () => {
      expect(() => service.createUser({  ...mockUser, cpf: '123'}))
        .toThrow(BadRequestException);
    });

    it('should throw if CPF has letters', () => {
      expect(() => service.createUser({ ...mockUser, cpf: '1234567890A'}))
        .toThrow(BadRequestException);
    });

    it('shoud throw if email is invalid', () => {
      expect(() => service.createUser({ ...mockUser, email: 'emailinvalid'}))
        .toThrow(BadRequestException);
    });

    it('should throw if password is less than 6 characters', () => {
      expect(() => service.createUser({ ...mockUser, password: '123'}))
        .toThrow(BadRequestException);
    });

    it('should throw if CPF already registered', () => {
      service.createUser(mockUser);
      expect(() => service.createUser({ ...mockUser, email: 'outro@gmail.com'}))
        .toThrow(BadRequestException);
    });

    it('should throw if email already registered', () => {
      service.createUser(mockUser);
      expect(() => service.createUser({ ...mockUser, cpf: '99999999999'}))
        .toThrow(BadRequestException)
    });

    describe('getAllUsers', () => {
      it('should return empety array initially', () => {
    ...
    it('should return users after creating', () => {
      service.createUser(mockUser);
      service.createUser({ ...mockUser, cpf: '99999999999', email: 'email@outro.com'})
    ...
    it('should throw if user not found', () => {
      expect(() => service.getUserById('id-inexistente'))
  ...
  describe('deleteUser', () => {
    ...
    it('should throw if user not found', () => {
      expect(() => service.deleteUser('id-inexistente'))
        .toThrow(NotFoundException);
    });
  });
  })
